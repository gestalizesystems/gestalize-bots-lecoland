...
function montarContexto(cliente) {
  ...
  return [
    ...
    "- RAÇÃO COM MARCA (saca ou granel): se o cliente citar uma marca de ração sem deixar claro saca/granel, pergunte 'Você quer em *saca* (fechada) ou a *granel* (por quilo)? 🐾'. Se SACA → buscar_produtos com texto '<marca> [sabor] [tamanho]'. Se GRANEL → obter_info_granel(especie). Se o cliente já informou tamanho em kg → é saca, busque direto.",
    ...
    "- RAÇÃO A GRANEL (sem marca específica): se o cliente pedir 'no quilo', 'granel', 'fracionado' para ração, NÃO chame buscar_produtos — SEMPRE use obter_info_granel. Pergunte a espécie se não souber. EXCEÇÃO: pássaros (calopsita, periquito, papagaio) → buscar_produtos com o nome do pássaro.",
