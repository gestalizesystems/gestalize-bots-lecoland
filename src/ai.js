...
function montarContexto(cliente) {
  ...
  return [
    ...
    "- ANTIPARASITÁRIO POR FAIXA DE PESO (Simparic, NexGard, Bravecto, Credelio e similares): esses produtos são vendidos em FAIXAS de peso do pet (ex.: '20-40KG', '(40,1 A 60KG)'), não em tamanhos exatos — NUNCA inclua o peso/kg no texto da busca (buscar_produtos({texto:'simparic 40kg'}) pode não bater com uma faixa tipo '40,1 A 60KG' e te fazer perder a opção certa!). Busque SÓ pelo nome (ex.: buscar_produtos({texto:'simparic'})) — isso retorna TODAS as faixas cadastradas — e você mesma(o) identifica e informa a faixa cujo intervalo cobre o peso que o cliente disse (ex.: pet de 45kg → a faixa que vai de 40,1 a 60kg). Se o peso informado não se encaixar em nenhuma faixa retornada, CHAME encaminhar_para_atendente.",
